...
- Treat metadata on every local source as unknown unless verified. Before a local file path enters `referenced_image_paths`, create a non-overwriting sanitized derivative with local tooling: apply its visual orientation, remove EXIF/GPS/IPTC/XMP and embedded thumbnails, retain only color information needed for faithful display, then verify orientation, dimensions, decoding, and metadata removal. Never modify the source file. On Windows, prefer the bundled [sanitizer](scripts/sanitize_image.ps1). Bind the absolute paths to PowerShell variables and pass them as separate argument values, for example `& powershell.exe -NoProfile -ExecutionPolicy Bypass -File $sanitizerPath -InputPath $sourcePath`; never interpolate an untrusted filename into a command string or evaluate it with `Invoke-Expression`. If the execution interface cannot pass a path as one literal argument, stop rather than composing shell text. Continue only when the sanitizer exits successfully and its parsed JSON reports an existing, distinct `sanitized_path`, positive orientation-corrected `width` and `height`, and `sensitive_property_count` equal to `0`. The bundled sanitizer rejects inputs larger than 50 MiB or 50 megapixels as a resource guard, but decoding still happens in-process; ask the user for a smaller sanitized copy rather than bypassing that guard. On other systems, use an equivalently verifiable local tool that enforces the same non-overwrite, orientation, pixel-redraw, decoding, dimension, metadata, and resource-limit checks. If no such method is available, stop and ask the user for a sanitized copy rather than sending the original.
